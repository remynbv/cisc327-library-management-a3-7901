...
def test_calculate_fees_book_one_day_overdue():
    """Calculate fees for book 1 day overdue."""
    insert_borrow_record("858585", 11, datetime.now() + timedelta(days=15), datetime.now() - timedelta(days=1))
    result = calculate_late_fee_for_book("858585", 11)
...
def test_calculate_fees_book_overdue_nine(): 
    ...
    insert_borrow_record("868686", 4, datetime.now() - timedelta(days=23), datetime.now() - timedelta(days = 9))
    result = calculate_late_fee_for_book("868686", 4)
...
def test_calculate_fees_book_overdue_forty_one(): 
    ...
    insert_borrow_record("878787", 4, datetime.now() - timedelta(days=55), datetime.now() - timedelta(days = 41))
    result = calculate_late_fee_for_book("878787", 4)
